...
import ImageTextLayout from '../../components/ImageTextLayout.astro';
...
**Telemetry** is data emitted from applications for the purposes of monitoring and observability. This includes logs, traces, and metrics.

**Observability** is using telemetry to observe a system - enabling investigations of issues. Think reviewing your logs to debug an issue.
...
Logs are useful for monitoring system-level events (i.e. startup, shutdown, etc.), but quickly become a burden when used for application-level events (i.e. API requests, database queries, etc.).
...
Here's an example span with custom attributes looks like.
...
Events are basically logs, but they are attached to the current span. For example, if you emit a critical error without any context, you'd struggle to determine where the error occurred. With events, the error is attached to the span, providing context for the error.
...
Metrics are simple numbers emitted from a system. They are much lighter and faster than logs or traces. Common use-cases for metrics include: system resource counters (CPU, memory, disk), active connections, and request counts.
...
Prior to OpenTelemetry, telemetry was a bit of a wild west. There was no standardized way to instrument, generate, collect, and export telemetry, leading to every vendor coming up with their own format.

OpenTelemetry is relatively new, but is quickly becoming the industry standard for telemetry.
...
The auto-instrumentation is super powerful. This automatically generates traces for incoming API requests, spans for database queries (showing the general query being executed), and spans for outgoing HTTP requests.
...
There are also many standalone platforms that support OpenTelemetry. In my experience, they're generally more expensive than the cloud providers, but they are easier to use and offer more features.
...
Robust telemetry enabling insightful monitoring and observability is critical for production systems. Invest the time now to integrate telemetry into your projects and your future self will thank you.
