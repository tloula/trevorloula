...
Mastering keyboard shortcuts will transform your relationship with your IDE - going from a toddler awkwardly blundering around to a ninja gracefully manipulating files. Alas, a video is worth a thousand words and I like to keep things short and to the point - check difference:
...
Here are some of my favorite shortcuts that I use all the time. If you aren't already familiar with them, I'd recommend trying them out as you read through the rest of the post.
...
Here's where the magic starts. `ALT + UP` will move the line up. `ALT + DOWN` will move the line down.
...
- `SHIFT + ALT + UP/DOWN` will add a new cursor above/below the current cursor.
