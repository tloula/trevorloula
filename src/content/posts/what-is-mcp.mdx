...
I've seen a lot of confusion around MCP. What is MCP? Does MCP replace tool calls? Why use MCP over just an API?
...
1. **Agent Framework**: This is the software layer that executes the agent (Cursor, GitHub Copilot, etc.).
...
> A note about LLM "memory": Many people think that LLMs themselves remember their conversation history. This is not the case. The LLM itself is stateless. The client application (ChatGPT UI, Cursor, GitHub Copilot, etc.) saves the conversation history and feeds it to the LLM with each new request.
